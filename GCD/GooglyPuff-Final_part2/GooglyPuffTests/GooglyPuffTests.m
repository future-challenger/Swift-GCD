...
{
    dispatch_semaphore_t semaphore = dispatch_semaphore_create(0); // 1
    dispatch_time_t timeoutTime = dispatch_time(DISPATCH_TIME_NOW, kDefaultTimeoutLengthInNanoSeconds); // 2
    ...
    __unused Photo *photo = [[Photo alloc]
                             ...
                             withCompletionBlock:^(UIImage *image, NSError *error) {
                                 ...
                                 dispatch_semaphore_signal(semaphore); // 3
                             }];
    
    if (dispatch_semaphore_wait(semaphore, timeoutTime)) { // 4
